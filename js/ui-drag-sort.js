$.fn.kpdragsort = function(options) {
	...
	$(container).children().off("mousedown").mousedown(function(e) {
		...
		var tempLeft = content ? content.offset().left : 0;
		var tempTop = content ? content.offset().top : 0;
		console.log(left);
		// 添加虚线框
		_this.before('<li id="kp_widget_holder"></li>');
		var wid = $("#kp_widget_holder");
		wid.css({"border":"2px dashed #ccc", "height":_this.outerHeight(true)-4,'margin-bottom':'17px'});
		...
		$(document).mouseup(function() {
			...
			// 拖拽回位，并删除虚线框
			var p = wid.offset();
			_this.animate({"left":left, "top":p.top}, 100, function() {
